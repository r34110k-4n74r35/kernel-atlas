...
def _structure_target(conn: sqlite3.Connection, row: sqlite3.Row) -> Target:
    target = _sym_target(conn, row)
...
def _structure_name_sql() -> str:
    return ("(s.name=? OR EXISTS (SELECT 1 FROM type_aliases a "
...
def _aggregate_ambiguity_note(candidates: list[Target], subject: str) -> str:
    if len(candidates) < 2:
        return ""
    return (f"{len(candidates)} aggregate definitions match {subject}; "
...
def resolve_structure(conn: sqlite3.Connection, spec: str) -> Resolution:
    ...
    if ":" in raw:
        ...
        if files and line is not None:
            ...
            rows = conn.execute(
                _STRUCT_SELECT + f" WHERE {kind_sql} AND s.file_id IN ({placeholders})"
                " AND s.start_line<=? AND s.end_line>=?",
                (*kind_params, *ids, line, line),
            ).fetchall()
            candidates = sorted(
                (_structure_target(conn, row) for row in rows),
                key=lambda target: (
            ...
            if candidates:
                note = (_aggregate_ambiguity_note(
                    candidates, f"line {line} in {head_n or 'the selected file'}")
    ...
    candidates = sorted(
        (_structure_target(conn, row) for row in rows), key=_rank_candidate)
    if not candidates:
        return Resolution(
            None, note=f"no struct/union tag or typedef alias is named {raw!r}")
...
def structure_detail(conn: sqlite3.Connection, target: Target) -> dict:
    ...
    if target.kind != "symbol" or target.symbol_kind not in {"struct", "union"}:
        raise ValueError("target is not a struct or union definition")
    symbol = conn.execute(
        "SELECT signature,summary,description,is_anonymous,parse_complete,parse_warnings,"
        " unmatched_member_docs,conditions FROM symbols WHERE id=?", (target.id,)
    ).fetchone()
    if symbol is None:
        raise ValueError("aggregate definition is missing from the index")

    aliases = [row["name"] for row in conn.execute(
    ...
    rows = conn.execute(
        ...
        " FROM type_members WHERE symbol_id=? ORDER BY ordinal", (target.id,)
    ).fetchall()
    ...
    described = sum(row["name"] is not None and row["description"] is not None
                    for row in rows)
    source_documentation = {
        "kernel-doc", "inline-kernel-doc", "source-comment",
    }
    documented = sum(
    ...
    semantic_descriptions = sum(
        row["description_source"] == "macro-semantics"
        for row in rows
    )
    ...
    return {
        ...
        "documentation_coverage": (
            documented / documentable if documentable else 0.0),

...
def render_structure(detail: dict, color: bool, max_width: int = 0) -> str:
    ...
    if index:
        title += f"  [Linux {index}]"
    out.write(paint(title, "1;36", color) + "\n")
    ...
    if unclassified:
        if unclassified.get("unmatched"):
            ...
            field("ownership", "primary only through "
                  + unclassified["maintainers_section"] + " catch-all")
    ...
    semantic_note = (f"; {semantic} parser-supplied macro explanation"
                     f"{'s' if semantic != 1 else ''}" if semantic else "")
    field(
        "members",
    ...
    if detail.get("summary"):
        out.write("\n" + paint("  Summary", "1", color) + "\n")
        out.write(textwrap.fill(
            detail["summary"], width=width, initial_indent="    ",
            subsequent_indent="    ") + "\n")
    if detail.get("description"):
        out.write("\n" + paint("  Description / notes", "1", color) + "\n")
        for paragraph in detail["description"].split("\n\n"):
            out.write(textwrap.fill(
                paragraph, width=width, initial_indent="    ",
                subsequent_indent="    ") + "\n")

    out.write("\n" + paint("  Members (source order)", "1", color) + "\n")
    ...
    def walk(members: list[dict], depth: int = 0) -> None:
        for member in members:
            ...
            if member.get("array_dimensions"):
                dims = "".join(f"[{value}]" for value in member["array_dimensions"])
            ...
            heading = f"{indent}{member['ordinal'] + 1:>3}. {name}{suffix}  [line {line}]"
            out.write(paint(heading, "33" if depth else "1;33", color) + "\n")
            ...
            def member_field(label: str, value) -> None:
                ...
                out.write(textwrap.fill(
                    str(value), width=width, initial_indent=prefix,
    ...
    if detail.get("unmatched_member_docs"):
        out.write("\n" + paint("  Documented but not matched", "1;33", color) + "\n")
        for name, description in detail["unmatched_member_docs"].items():
            out.write(textwrap.fill(
                f"@{name}: {description}", width=width,
                initial_indent="    ", subsequent_indent="    ") + "\n")
    if detail.get("warnings"):
        out.write("\n" + paint("  Parse warnings", "1;33", color) + "\n")
        for warning in detail["warnings"]:
            out.write(f"    - {warning}\n")

    docs = detail.get("related_documentation", [])
    if docs:
        out.write("\n" + paint("  Related Documentation", "1", color) + "\n")
        for item in docs:
